import subprocess, os, time, socket, struct
...
class ping_loop():
	...
	def number_ip(self, iprange1, iprange2):
		...
		for i in range(0,4):
			if ip_range1[i] == ip_range2[i]:
				iprange = None
			if ip_range1[i] < ip_range2[i]:
				iprange = int(ip_range2[i]) - int(ip_range1[i])
			if ip_range1[i] > ip_range2[i]:
				iprange = int(ip_range1[i]) - int(ip_range2[i])

			# if ip is determine
			if iprange != None:
				if i == 0:
					# xxx.000.000.000 range (^32)
					iprange0 = iprange*254*254*254
					iprange0 += int(iprange0)
					list_numb.append(iprange0)

				if i == 1:
					# xxx.xxx.000.000 range (^24)
					iprange1 = iprange*254*254
					iprange1 += int(iprange1)
					list_numb.append(iprange1)

				if i == 2:
					# xxx.xxx.xxx.000 range (^16)
					iprange2 = iprange*254
					iprange2 += int(iprange2)
					list_numb.append(iprange2)

				if i == 3:
					# xxx.xxx.xxx.xxx range (^8)
	...
	def ping(self, ip_start, ip_stop):
		...
		print (" start {0}".format(ip_start))
		print (" end {0}".format(ip_stop))
